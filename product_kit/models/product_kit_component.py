...
class ProductKit(models.Model):
    ...
    product_id = fields.Many2one(
        comodel_name='product.product', string='Kit', required=True, ondelete='cascade')
    ...
    def _compute_price(self):
        for kit_component in self:
            ...
            if component_currency_id.id and product_currency.id != component_currency_id.id:
                kit_price_component_currency = component_currency_id._convert(
                    kit_component.product_component_id.list_price, product_currency, kit_component.company_id.id, fields.Datetime.today())
            else:
